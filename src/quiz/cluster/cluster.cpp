...
void Proximity(const std::vector<std::vector<float>> &points, int point_id, KdTree *tree, float distanceTol, std::vector<int> *cluster, std::set<int> *processed_points)
{
	processed_points->insert(point_id);
	cluster->push_back(point_id);
	std::vector<int> nearby_points = tree->search(points[point_id], distanceTol);
	for (int nearby_i : nearby_points)
	{
		if (processed_points->count(nearby_i) == 0)
		{
			Proximity(points, nearby_i, tree, distanceTol, cluster, processed_points);
		}
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>> &points, KdTree *tree, float distanceTol)
{
	...
	{
		if (processed_points.count(i) == 0)
		{
			std::vector<int> new_cluster;
			Proximity(points, i, tree, distanceTol, &new_cluster, &processed_points);
